...
const SpeechItem: React.FC<SpeechItemProps> = ({ type }) => {
  ...
  return (
    <>
      ...
      {/* type = 3 名片 */}
      {type === 3 && (
        <Form.Item label="名片样式" name="contentObj.pic">
      ...
      {/* type = 4 小站 */}
      {type === 4 && (
        <Form.Item label="小站样式" name="contentObj.pic">
      ...
      {type === 8 && (
        <>
          ...
          <Form.Item label="图文标题" rules={[{ required: true }]} name="contentObj.title">
            <InputShowLength className="width480" maxLength={30} placeholder={'请输入'} />
          </Form.Item>
          <Form.Item label="图文摘要" name="contentObj.summary" rules={[{ required: true }]}>
            <InputShowLength className="width480" maxLength={30} placeholder={'请输入'} />
          </Form.Item>
          <Form.Item label="图文链接" name="contentObj.contentUrl" rules={[{ required: true }]}>
            <Input className={'width480'}></Input>
      ...
      {(type === 6 || type === 7) && (
        <>
          ...
          <Form.Item label="摘要" name="contentObj.summary" rules={[{ required: true }, { max: 30 }]}>
            <InputShowLength className="width480" maxLength={30} placeholder={'请输入'} />
          </Form.Item>
        </>
      )}
      ...
      {type === 9 && (
        <>
          <Form.Item name="contentObj.appId" label="小程序Id" rules={[{ required: true }, { max: 40 }]}>
            <Input className="width480" placeholder={'请输入'} />
          </Form.Item>
          <Form.Item name="contentObj.appPath" label="路径">
            <Input className="width480" placeholder={'请输入'} />
          </Form.Item>
          <Form.Item
            label={'上传图片'}
          ...
          <Form.Item name="contentObj.title" label="小程序标题" rules={[{ required: true }, { max: 30 }]}>
            <InputShowLength className="width480" maxLength={30} placeholder={'请输入'} />
          </Form.Item>
          <Form.Item name="contentObj.summary" label="小程序摘要" rules={[{ required: true }, { max: 30 }]}>
            <InputShowLength className="width480" maxLength={30} placeholder={'请输入'} />
          </Form.Item>
        </>
      )}
      {type === 10 && (
        <>
          ...
          <Form.Item label={'上传PDF'} name="contentObj.contentUrl" rules={[{ required: true }]}>
            <NgUploadFile />
          </Form.Item>
          <Form.Item name="contentObj.title" label="PDF标题" rules={[{ required: true }, { max: 30 }]}>
            <InputShowLength className="width480" maxLength={30} placeholder={'请输入'} />
          </Form.Item>
          <Form.Item name="contentObj.summary" label="PDF摘要" rules={[{ required: true }, { max: 30 }]}>
            <InputShowLength className="width480" maxLength={30} placeholder={'请输入'} />
          </Form.Item>
        </>
      )}
