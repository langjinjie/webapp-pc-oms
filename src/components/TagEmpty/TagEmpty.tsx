...
const TagEmpty: React.FC<TagEmptyProps> = ({ type, className }) => {
  const typeToData: any = {
    tag: {
      imgUrl: require('src/assets/images/exception/tag.png'),
      mainText: '还未添加任何标签',
      deputyText: '快来给你的客户添加点标签吧～'
    },
    service: {
      imgUrl: require('src/assets/images/exception/service.png'),
      mainText: '暂无服务建议',
      deputyText: '该客户无任何标签，去给客户添加标签吧！'
    },
    portrait: {
      imgUrl: require('src/assets/images/exception/portrait.png'),
      mainText: '分析不出该客户画像',
      deputyText: '该客户无任何标签，去给客户添加标签吧！'
    },
    movement: {
      imgUrl: require('src/assets/images/exception/movement.png'),
      mainText: '暂无动态',
  ...
  return (
    <div className={classNames(style.tagEmptyWrap, className)}>
      ...
      <div className={style.deputyText}>{typeToData[type].deputyText}</div>
    </div>
  );
};
